...
JNIEXPORT void JNICALL Java_com_ibm_jusb_os_linux_JavaxUsb_nativeSubmitInterruptRequest
  ( JNIEnv *env, jclass JavaxUsb, jobject localLinuxPipeRequest, jbyte epAddress)
{
	...
	if (!(urb = malloc(sizeof(*urb)))) {
		dbg( MSG_CRITICAL, "nativeSubmitInterruptRequest : Out of memory! (%d needed)\n", sizeof(*urb) );
	...
	dbg( MSG_DEBUG2, "nativeSubmitInterruptRequest : Submitting URB\n" );
	...
	debug_urb( "nativeSubmitInterruptRequest", urb );
	...
	if (result) {
		dbg( MSG_ERROR, "nativeSubmitInterruptRequest : Could not submit URB (errno %d)\n", result );
		(*env)->ReleaseByteArrayElements( env, data, urb->buffer, 0 );
		(*env)->CallVoidMethod( env, linuxRequestProxy, removePendingVector, linuxPipeRequest );
		free(urb);
	} else {
		dbg( MSG_DEBUG2, "nativeSubmitInterruptRequest : Submitted URB\n" );
  ...
  ( JNIEnv *env, jclass JavaxUsb, jobject linuxPipeRequest )
{
	...
	dbg( MSG_DEBUG2, "nativeCompleteInterruptRequest : Completing URB\n" );

	if (!(urb = (struct usbdevfs_urb*)(*env)->CallIntMethod( env, linuxPipeRequest, getUrbAddress ))) {
		dbg( MSG_ERROR, "nativeCompleteInterruptRequest : No URB to complete\n" );
		(*env)->CallVoidMethod( env, linuxRequestProxy, removePendingVector, linuxPipeRequest );
